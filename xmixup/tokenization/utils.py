...
from xmixup.utils.file_utils import (
    add_end_docstrings,
    cached_path,
    hf_bucket_url,
    is_remote_url,
...
class PreTrainedTokenizerBase(SpecialTokensMixin):
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        if pretrained_model_name_or_path in s3_models:
            ...
            if is_remote_url(pretrained_model_name_or_path):
                ...
                for file_id, file_name in {**cls.vocab_files_names, **additional_files_names}.items():
                    if tf.io.gfile.isdir(pretrained_model_name_or_path):
                        full_file_name = os.path.join(pretrained_model_name_or_path, file_name)
                        if not tf.io.gfile.exists(full_file_name):
        ...
        if tokenizer_config_file is not None:
            with tf.io.gfile.GFile(tokenizer_config_file, "r") as tokenizer_config_handle:
        ...
        if special_tokens_map_file is not None:
            with tf.io.gfile.GFile(special_tokens_map_file, "r") as special_tokens_map_handle:
        ...
        if added_tokens_file is not None:
            with tf.io.gfile.GFile(added_tokens_file, "r") as added_tokens_handle:
    ...
    def save_pretrained(self, save_directory) -> Tuple[str]:
        ...
        """
        if not tf.io.gfile.isdir(save_directory):
        ...
        with tf.io.gfile.GFile(tokenizer_config_file, "w") as f:
            f.write(json.dumps(tokenizer_config, ensure_ascii=False))

        with tf.io.gfile.GFile(special_tokens_map_file, "w") as f:
            write_dict = {}
        ...
        if added_vocab:
            with tf.io.gfile.GFile(added_tokens_file, "w") as f:
    ...
    def _batch_encode_plus(
        ...
        **kwargs
        ...
        for ids_or_pair_ids in batch_text_or_text_pairs:
            ...
            if first_ids is None and second_ids is not None:
