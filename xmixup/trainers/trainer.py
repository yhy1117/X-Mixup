...
from xmixup.models.optimization import GradientAccumulator, create_optimizer
...
class Trainer:
    ...
    def get_optimizers(
        ...
        warmup_steps = max(self.args.warmup_steps, int(num_training_steps * self.args.warmup_prop))
    ...
    def evaluate(self, eval_dataset: Optional[tf.data.Dataset] = None, final_eval: Optional[bool] = False) \
            -> Dict[str, float]:
        """
        ...
        if not final_eval:
            ...
            output = self._prediction_loop(eval_ds, description="Final evaluation")
