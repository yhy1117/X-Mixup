"""Tensorflow trainers class for tagging tasks."""
...
from xmixup.utils.train_utils import PREFIX_CHECKPOINT_DIR, EvalPrediction, PredictionOutput, is_wandb_available, set_seed
...
class TaggingTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset: Optional[tf.data.Dataset] = None, final_eval: Optional[bool] = False) -> Dict[str, float]:
        ...
        logs, output_metrics_dict = {}, {}
        if final_eval:
            description = "Final evaluation"
        else:
            description = "Evaluation"

        for lang, ds in zip(self.eval_languages, eval_ds):
            output = self._prediction_loop(ds, description=description)
            output_metrics = output.metrics
            for key in list(output_metrics.keys()):
                if key not in output_metrics_dict:
                    output_metrics_dict[key] = output_metrics[key]
                else:
                    output_metrics_dict[key] += output_metrics[key]

        n_langs = len(self.eval_languages)
        for key in list(output_metrics_dict.keys()):
            output_metrics_dict[key] = output_metrics_dict[key] / n_langs
        ...
        for epoch_iter in range(epochs_trained, int(epochs + 1)):
            ...
            for step, training_loss in enumerate(self._training_steps(train_ds, optimizer)):
                ...
                if (
                    ...
                    logs["trg_task_loss"] = training_loss[3].numpy()
    ...
    def _accumulate_gradients(self, per_replica_features, per_replica_labels):
        ...
        try:
            ...
            reduced_trg_task_loss = self.args.strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_detailed_loss[2],
                                                          axis=0)

            return reduced_loss, reduced_task_loss, reduced_consist_loss, reduced_trg_task_loss
    ...
    def _run_model(self, features, labels, training):
        ...
        if isinstance(labels, (dict)):
            ...
            outputs = self.model(features, labels=labels, training=training, mixup_inference=self.args.mixup_inference,
                                 ...
                                 sample_gold=sample_gold)
        if training:
            loss, logits, task_loss, consist_loss, trg_task_loss = outputs
        else:
        ...
        if training:
            return loss, logits, [task_loss, consist_loss, trg_task_loss]
        else:
            return loss, logits
